...
$_G = array();
$_G['ucenter'] = include MUDDER_ROOT.'./data/cachefiles/ucenter_config.php';
if(!$_G['ucenter']['uc_enable']) exit('Invalid Request');
...
class uc_note {
	...
	function updatecredit($get, $post) {
        ...
        $this->db->exec("UPDATE {$this->dbpre}members SET coin=coin+'$amount' WHERE uid='$uid'");
	...
	function getcredit($get, $post) {
        ...
        $q = $this->db->query("SELECT coin FROM {$this->dbpre}members WHERE uid='$uid'");
        $r = $q->fatch_array();
        return (int) $r['coin'];
	}
