...
function deletemsg(pmid) {
    ...
    $(document.body).append('<form method="post" id="deletemsg" action="<?php echo url("member/index/ac/{$ac}/op/delete"); ?>" style="display:none;"></form>');
...
<? include template('menu','member','member'); ?>
        ...
        <div class="myright">
            ...
            <? if(defined('IN_UC')) { ?>
            <iframe src="<?php echo url("member/index/ac/pm/op/uc"); ?>" id="ucpm" style="width:100%;height:550px;border:0;" frameBorder="0"></iframe>
            
<? } else { ?>
                <h3>我的短信箱</h3>
                <div class="mainrail">
                    <ul class="optabs"><li<? if($folder==inbox) { ?>
 class="active"<? } ?>
><a href="<?php echo url("member/index/ac/{$ac}/folder/inbox"); ?>">收件箱</a></li><li<? if($folder==outbox) { ?>
 class="active"<? } ?>
><a href="<?php echo url("member/index/ac/{$ac}/folder/outbox"); ?>">发件箱</a></li><li class="act"><a href="<?php echo url("member/index/ac/{$ac}/op/write"); ?>">发送短信息</a></li></ul><div class="clear"></div>
                    <form method="post" action="<?php echo url("member/index/ac/{$ac}/op/delete"); ?>" onsubmit="return checkbox_check();">
...
<? include template('pm_message_tr', 'member', MOD_FLAG); ?>
                    </table>
                    <? if($total) { ?>
