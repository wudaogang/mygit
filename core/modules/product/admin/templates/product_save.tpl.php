...
function product_create_category(sid) {
    ...
    if(!catname) return;
	$.post("<?=cpurl($module,$act,'create')?>", {'sid':sid, 'catname':catname, 'in_ajax':1 }, 
		function(result) {
...
function select_subject(obj) {
	var url = "<?=cpurl($module,$act,'add')?>"+'&sid='+$(obj).attr('sid');
